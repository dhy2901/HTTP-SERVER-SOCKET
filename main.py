...
def read_Config_File(filename):
    ...
    try:
        ...
        cache_time = int(config["ProxyConfig"]["cache_time"]) #ProxyConfig là 1 header trong file.ini lấy nội dung từ catch_time chuyển từ string sang int
        whitelisting = [
            domain.strip() #Loại bỏ khoảng trắng
            for domain in config["ProxyConfig"]["whitelisting"].split(",") #Lưu lần lượt các domain trong list
        ]
        time_range = [int(t) for t in config["ProxyConfig"]["time"].split("-")] #Tách thời gian start-end từ string sang int [7,23]
...
def error_403_html(file_path):
    ...
    """
    try:
        with open(file_path, "rb") as file: #Mở  file cho chế độ định dạng nhị phân
            error_data = b"HTTP/1.1 403 Forbidden\r\nContent-Type: text/html\r\n\r\n" #b là mở file ở chế độ nhị phân
            # tạo một mảng bytes (bytes) chứa thông tin về phản hồi lỗi 403 và loại nội dung là HTML. Đây là phần đầu của phản hồi HTTP.
            error_data += file.read() #mở file 403.html để thêm vào error_data tạo thành nội dung hoàn chỉnh
        return error_data
    except Exception as Error:
    ...
    try:
        ...
        if client_data:
            ...
            if method == None or method.upper() not in ACCEPT_METHOD or not is_whitelisted(url, whitelisting) or not available_time_range(time_range):
                #  server gửi một phản hồi lỗi 403 Forbidden cho client bằng cách gọi hàm error_403_html() và truyền tệp HTML chứa nội dung lỗi "403.html".
                client_socket.sendall(error_403_html("403.html"))
                client_socket.close() #Đóng quá trình trao đổi giữa client và server
                return
